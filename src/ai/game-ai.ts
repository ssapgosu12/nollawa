import { moveKey, type AiGameId, type GameMove, type GameState } from '../game/catalog';
...
export function fallbackGameMove(id: AiGameId, state: GameState): GameMove | null {
...
export function chooseGameMoveDetailed(id: AiGameId, state: GameState, budgetMs: number, now: () => number = () => performance.now()): SearchResult {
...
export const chooseGameMove = (id: AiGameId, state: GameState, budgetMs: number): GameMove | null => chooseGameMoveDetailed(id, state, budgetMs).move;
