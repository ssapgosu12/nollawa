...
interface FleetVariantSelection { targetParticipantId: string; cell: FleetCell }
...
const FLEET_SHOT_LABELS: Record<FleetShootingCard, string> = {
  salvo: '일제사격탄', flare: '조명탄', tracer: '예광탄', 'high-explosive': '고폭탄',
  scatter: '확산탄', piercing: '관통탄', 'random-shot': '무작위탄', buckshot: '산탄',
};

export interface FleetScoreSheetRow { id: string; name: string; remainingShips: number; shotType: string }

export function fleetScoreSheetRows(state: FleetState): readonly FleetScoreSheetRow[] {
  return state.participants.map((participant) => {
    const hits = new Set(state.shots.filter(({ target, result }) => target === participant.id && result !== 'miss').map(({ cell }) => key(cell)));
    const remainingShips = participant.ships.filter((ship) => !ship.sunk && !ship.cells.every((cell) => hits.has(key(cell)))).length;
    const shootingCard = participant.variantSetup?.shootingCard;
    return { id: participant.id, name: participant.name, remainingShips, shotType: shootingCard ? FLEET_SHOT_LABELS[shootingCard] : '일반탄' };
  });
}
...
export function FleetGame({ state, viewerId, onAction, onExit }: Props) {
  ...
  const [variantSelections, setVariantSelections] = useState<FleetVariantSelection[]>([]), [spread, setSpread] = useState(false);
  ...
  const sheetRows = fleetScoreSheetRows(state);
  ...
  const selectTarget = (cell: FleetCell) => { if (selectionNeeded && target) setVariantSelections((current) => current.length < selectionNeeded ? [...current, { targetParticipantId: target.id, cell }] : current); };
      ...
      const plan: FleetShotPlan = bonusMode === 'carrier' ? { type: 'normal', cell: confirmableTarget.cell }
        : bonusMode === 'tracer' ? { type: 'tracer', center: confirmableTarget.cell }
          : bonusMode === 'pressure' ? { type: 'explosive', boardSize: state.boardSize, turnIndex: (state.round ?? 1) - 1, center: confirmableTarget.cell }
            : fleetVariantPlanForCell(setup.shootingCard, confirmableTarget.cell, state, confirmableTarget.targetParticipantId, Math.random, variantSelections.map(({ cell }) => cell), spread, viewerId ?? '');
      const flareTargets = !bonusMode && setup.shootingCard === 'flare' ? variantSelections.map(({ targetParticipantId }) => targetParticipantId) : null;
      onAction({ type: 'queue-variant-shot', targetParticipantId: flareTargets?.[0] ?? confirmableTarget.targetParticipantId, ...(flareTargets ? { targetParticipantIds: flareTargets } : {}), plan });
    }
    setTargetPreview(null); setVariantSelections([]); setBonusMode(null);
  };
  ...
  return <section class={`fleet-screen${sheetOpen ? ' sheet-active' : ''}`} aria-label="함대 격침">
    <div class="fleet-zone fleet-upper">
      ...
      <div class="fleet-board-shell">
        {state.phase === 'placement' && !sheetOpen
          ...
          : <><span class="fleet-board-name">{target?.name ?? '관전'}{target?.alive === false ? ' · 탈락' : ''}</span><FleetBoard state={state} participant={target} interactive={canShoot} selectedCell={confirmableTarget?.cell} selectedCells={[...selectedImpacts, ...selectedCells]}
    ...
    <div class="fleet-middle">
      {state.phase === 'placement' ? <>
        <div class="fleet-ship-picker" aria-label="배 선택">{variant ? fleetLengths.map((length, index) => <button class={`${shipIndex === index ? 'selected ' : ''}${own?.ships?.some(({ index: placedIndex }) => placedIndex === index) ? 'placed' : 'unplaced'}`} aria-pressed={shipIndex === index} aria-label={`${variantFleet[index]?.id ?? `${index + 1}번 함선`}, ${length}칸, ${own?.ships?.some(({ index: placedIndex }) => placedIndex === index) ? '배치 완료' : '미배치'}`} onClick={() => setShipIndex(index)} key={index}><span>{variantFleet[index]?.id}</span><small>{variantFleet[index]?.placementTag === 'coastal' ? '연안' : variantFleet[index]?.placementTag === 'ocean' ? '원양' : `${length}칸`} · {own?.ships?.some(({ index: placedIndex }) => placedIndex === index) ? '배치됨' : '미배치'}</small></button>) : CLASSIC_FLEET_LENGTHS.map((length, index) => <button class={`${shipIndex === index ? 'selected ' : ''}${own?.ships?.some(({ index: placedIndex }) => placedIndex === index) ? 'placed' : 'unplaced'}`} aria-pressed={shipIndex === index}
          aria-label={`${CLASSIC_FLEET_NAMES[index]}, ${length}칸, ${own?.ships?.some(({ index: placedIndex }) => placedIndex === index) ? '배치 완료' : '미배치'}`} onClick={() => setShipIndex(index)} key={index}><span>{CLASSIC_FLEET_NAMES[index]}</span><small>{length}칸 · {own?.ships?.some(({ index: placedIndex }) => placedIndex === index) ? '배치됨' : '미배치'}</small></button>)}</div>
        ...
        {variant && view.privateScouts?.map((scout) => <p class="fleet-private-scout" aria-live="polite" key={`${scout.round}:${scout.targetId}:${key(scout.cell)}`}>SCOUT {view.participants.find(({ id }) => id === scout.targetId)?.name ?? scout.targetId} {scout.cell.row + 1},{scout.cell.column + 1}: {scout.occupied ? 'OCCUPIED' : 'EMPTY'}</p>)}
        {variant && <>{setup?.shootingCard === 'buckshot' && <div class="fleet-buckshot-choice" role="radiogroup" aria-label="산탄 발사 선택"><button class={!spread ? 'selected' : ''} aria-pressed={!spread} onClick={() => setSpread(false)}>일반탄 1발</button><button class={spread ? 'selected' : ''} aria-pressed={spread} onClick={() => setSpread(true)}>산탄 6발</button></div>}{abilities?.carrierExtraShots ? <button onClick={() => setBonusMode('carrier')}>항모 추가탄</button> : null}{abilities?.tracerShots ? <button onClick={() => setBonusMode('tracer')}>추가 예광탄</button> : null}{abilities?.privateScouts ? <button onClick={() => setBonusMode('spy')}>비공개 정찰</button> : null}{abilities?.glassCannonPressure && (state.round ?? 1) % 2 === 1 ? <button onClick={() => setBonusMode('pressure')}>압박 고폭탄</button> : null}</>}
    ...
    <ScoreSheet open={sheetOpen} locked={false} onOpenChange={setSheetOpen} panelClass="fleet-sheet" handleClass="fleet-sheet-handle">
      <table><thead><tr><th>항목</th>{sheetRows.map((participant, index) => <th class={`player-${index + 1}`} key={participant.id}>{participant.name}</th>)}</tr></thead><tbody>
        <tr><th>남은 배</th>{sheetRows.map((participant) => <td key={participant.id}>{participant.remainingShips}척</td>)}</tr>
        <tr><th>탄 종류</th>{sheetRows.map((participant) => <td key={participant.id}>{participant.shotType}</td>)}</tr>
      </tbody></table>
    </ScoreSheet>
