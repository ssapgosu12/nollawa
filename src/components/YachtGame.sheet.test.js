...
const sharedSheet = readFileSync(new URL('./ScoreSheet.tsx', import.meta.url), 'utf8');
const yachtTable = readFileSync(new URL('./YachtScoreTable.tsx', import.meta.url), 'utf8');
...
describe('Yacht score sheet parent acceptance', () => {
  it('acceptance 5 overlays voluntary and forced sheets below a dice-only 15 percent viewport', () => {
    ...
    expect(css).toMatch(/\.score-sheet-panel \{[^}]*position: fixed;[^}]*top: 15svh;[^}]*bottom: 0;/);
    expect(css).toMatch(/\.score-sheet-handle \{[^}]*z-index: 10;/);
    expect(css).toMatch(/\.score-sheet-handle:disabled \{ display: none; \}/);
    expect(component).toContain("sheetOpen ? ' sheet-active' : ''");
    expect(component).toContain('locked={forced}');
    expect(component).toContain('onOpenChange={onSheetOpenChange}');
    expect(sharedSheet).toContain('onOpenChange(nextScoreSheetOpen(open, locked))');
  });
  ...
  it('acceptance 7 preserves seat columns and colors while current turn and self remain independent', () => {
    ...
    expect(yachtTable).toContain('yacht-self-marker');
