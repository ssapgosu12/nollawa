...
export function ForbiddenPoint({ kind, cx, cy }: { kind: string; cx: number; cy: number }) { return <g aria-label={`금수: ${kind}`} pointer-events="none"><circle cx={cx} cy={cy} r="18" fill="var(--paper)" stroke="var(--danger)" stroke-width="6" /><path d={`M${cx - 9} ${cy - 9}L${cx + 9} ${cy + 9}M${cx + 9} ${cy - 9}L${cx - 9} ${cy + 9}`} stroke="var(--danger)" stroke-width="5" /></g>; }
